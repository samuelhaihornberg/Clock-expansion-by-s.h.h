...
function startClock() {
  ...
  const bannerCountry = document.querySelector('#country');
  if (bannerName) bannerName.textContent = name;
  if (bannerCountry) bannerCountry.textContent = country;
  ...
  function numberedRing(x, y, radius, count, color, activeValue, size) {
    circle(x, y, radius, color, 0.8, 0.12);
    for (let value = 1; value <= count; value += 1) {
      const angle = -Math.PI / 2 + (value / count) * Math.PI * 2;
      const isActive = value === activeValue;
      label(value, x + Math.cos(angle) * radius, y + Math.sin(angle) * radius, color, isActive ? size + 1 : size);
    }
  }
  ...
  function draw(time) {
    ...
    const x = width / 2;
    const y = height / 2;
    const usable = Math.max(40, Math.min(width, height) / 2 - 32);
    ...
    const rings = calendarMode ? [
      [base, base * 2.8, day, colors.days, 2.5],
      [base * 2.8, base * 5.8, month, colors.months, 3.5],
      [base * 5.8, base * 13.5, year, colors.years, 4.5]
    ] : [
      [base, base * 2.4, time.ms / 1000, colors.ms, 2],
      [base * 2.4, base * 4.8, second, colors.seconds, 2.5],
      [base * 4.8, base * 7.8, minute, colors.minutes, 3],
      [base * 7.8, base * 10.8, hour, colors.hours, 3.5]
    ];

    rings.forEach(([inner, outer, progress, color, widthValue]) => {
      circle(x, y, inner, color, widthValue, 0.18);
      circle(x, y, outer, color, 1, 0.08);
      wave(x, y, inner, outer, progress, color, widthValue);
    });

    if (calendarMode) {
      numberedRing(x, y, base * 2.3, dayCount, colors.days, time.day, 8);
      numberedRing(x, y, base * 5.2, 12, colors.months, time.month, 9);
      numberedRing(x, y, base * 12.1, YEAR_END - YEAR_START + 1, colors.years, time.year, 5);
      plusLaser(x, y, base * 13.5, year, colors.years);
    } else {
      numberedRing(x, y, base * 2.1, 1000, colors.ms, Math.floor(time.ms), 4);
      numberedRing(x, y, base * 4.5, 60, colors.seconds, time.second, 7);
      numberedRing(x, y, base * 7.2, 60, colors.minutes, time.minute, 7);
      numberedRing(x, y, base * 10.2, 24, colors.hours, time.hour, 8);
      plusLaser(x, y, base * 10.8, hour, colors.hours);
    }
    ...
    context.beginPath();
    context.arc(x, y, base * 1.5, 0, Math.PI * 2);
    context.fill();
